...
abstract class CartStoreBase with Store {
  ...
  // Action: -----------------------------------------------------------------------------------------------------------
  @action
  Future<void> setCartKey(String value, [guest = true]) async {
    _cartKey = value;
    if (guest) {
      // cartKey when not logged in
      await _persistHelper.saveGuestCartKey(value);
    } else {
      await _persistHelper.saveCartKey(value);
    }
  }
  ...
  Future<void> mergeCart({bool? isLogin}) async {
    ...
    // successful login callback cart data
    await getCart();
  }
  ...
  Future<void> _restore() async {
    ...
    if (cartKey != null && cartKey != "") {
      //cartKey login
      _cartKey = _authStore.isLogin ? cartKey : guest;
      await setCartKey(_cartKey!, false);
    } else {
      // cartKey when not logged in
      String id = guest is String ? guest : StringGenerate.uuid();
      await setCartKey(id);
